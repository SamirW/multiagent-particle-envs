    ...
    def reset_world(self, world, flip=False, start_poses=None, goal_poses=None, start_vels=None):
        ...
        for i, agent in enumerate(world.agents):
            ...
            if start_poses is None:
                if self.mode == 0:
                    if i == flip:
                        agent.state.p_pos = np.array([np.random.uniform(3.5, 4.25), np.random.uniform(2, 2.25)])
                    else:
                        agent.state.p_pos = np.array([np.random.uniform(3.5, 4.25), np.random.uniform(-2.25, -2)])
                else:
                    pass
            else:
                for i, agent in enumerate(world.agents):
